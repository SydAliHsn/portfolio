  ...
  return (
    ...
    >
      ...
      <Link
        href={url}
        target="_blank"
        className="fadeIn md:w-[65%] md:rounded-3xl rounded-2xl overflow-hidden relative shadow-project bg-primary-light/70"
      >
        <Image
          src={img}
          alt={title}
          className="hover:scale-110 ease-in-out transition-transform duration-500"
        />
      </Link>
      ...
      <div
        className={`fadeIn rounded-3xl md:absolute ${reverse ? 'left-0' : 'right-0 md:text-right'} top-1/2 md:-translate-y-1/2 md:w-[28rem] lg:w-[30rem] xl:w-1/2`}
      >
        ...
        <p
          className="text-sm lg:text-base bg-lightest dark:bg-dark rounded-3xl md:p-6 dark:text-light
    md:shadow-md shadow-light/10 dark:shadow-black/10 dark:shadow-black/10/10"
        >
          {content}
        </p>
        ...
        <ul className={`flex flex-wrap mt-4 gap-[0.6rem] ${!reverse && 'md:justify-end'}`}>
          {technologies.map(tech => (
            <li
              key={tech}
              className="md:bg-lighter md:dark:bg-darkest !bg-opacity-70 border-2 px-2 py-1 rounded-2xl border-light text-dark dark:text-lighter md:text-sm text-xs"
            >
              {tech}
            </li>
          ))}
        ...
        <div className="md:hidden flex flex-wrap justify-center xs:justify-start gap-2 xs:gap-4 mt-8 lg:mt-14">
          <PrimaryButton
            href={`https://github.com/sydalihsn/${repoName}`}
        ...
        <div
          className={`hidden md:flex text-dark/90 dark:text-light gap-5 text-2xl pt-4 ${!reverse && 'md:justify-end'}`}
        >
          <Link
            className="duration-200 hover:scale-110 hover:text-primary-light dark:hover:text-secondary-dark"
            href={`https://github.com/sydalihsn/${repoName}`}
            target="_blank"
          >
            <BiLogoGithub />
